...
function findStep(slug: string) {
  return newApartmentSteps.find((s) => s.slug === slug);
}

export async function generateStaticParams() {
  return newApartmentSteps.map((step) => ({ step: step.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!step) return {};
  return {
    title: `${step.title} - 새아파트 이사 ${step.order}단계`,
    description: step.subtitle + ". " + step.description.slice(0, 120),
  };
}

export default async function NewApartmentStepPage({ params }: Props) {
  const { step: slug } = await params;
  const step = findStep(slug);
  if (!step) notFound();
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Ad: 가이드 사이드바 — 비용 테이블 아래 자연스러운 위치 */}
      ...
      {/* Navigation */}
      <div className="mt-6">
        <StepNavigation currentStep={step} allSteps={newApartmentSteps} />
      </div>
